...
export default class VideoStatsMos {
  ...
  static STATS_LOG_TIME_SAMPLE = 1000
  ...
  static calculateVideoScore(currentStats, lastStats) {
    ...
    let bitrate = 8 * (currentStats.videoBytesReceived - lastStats.videoBytesReceived)
    bitrate *= (interval / 1000)

    const pixelCount = this.videoDimensions.e * this.videoDimensions.height
  ...
  onVideoStats(subscriberStats, callbackOutputResult) {
    ...
    if ((currentTimeMillis - this.statsWindowTestTime) > VideoStatsMos.SCORE_TIME_WINDOW) {
      let videoScoreAverage = 0
      for (let index = 1; index < this.statsLog.length; index += 1) {
        videoScoreAverage += VideoStatsMos.calculateVideoScore(this.statsLog[index], this.statsLog[index - 1])
      }
